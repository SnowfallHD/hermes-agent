...
KANBAN_GUIDANCE = (
    ...
    "task). It returns title/body, parent handoffs, retry history, comments, "
    "and `worker_context` as ground truth.\n"
    "2. **Work inside the workspace.** `cd $HERMES_KANBAN_WORKSPACE` before "
    "any file operations. The workspace is yours for this run. Don't modify "
    "files outside it unless the task explicitly asks.\n"
    "3. **Heartbeat on long operations.** Call `kanban_heartbeat(note=...)` "
    "every few minutes during long subprocesses; skip short tasks. **If your "
    "task may run longer than 1 hour, call `kanban_heartbeat` at least hourly** "
    "or the dispatcher may requeue stale tasks without penalty, losing this "
    "run's progress.\n"
    "4. **Block only on a real execution gate.** If you need a human decision you "
    "cannot infer (credentials, UX choice, paywalled source, required peer "
    "output), call `kanban_block(reason=\"...\")` and stop. Don't guess. Do not "
    "block solely because you produced a local/internal/draft-only artifact or "
    "optional future external action. Do not block solely because the artifact is code. "
    "If acceptance criteria are met and no "
    "permission is needed, complete the task; note that publishing/outreach/spend/"
    "account/production actions were not performed. If task or board policy "
    "requires review/proof, use the configured review/block workflow.\n"
    ...
    "Downstream workers read both. Never put secrets/tokens/raw PII there — "
    "run rows are durable forever. Review is not a synonym for human approval: "
    "for a local/internal/reversible code change with requested proof, complete "
    "the task with structured metadata. Route a reviewer child if useful. Use "
    "`kanban_block(reason=\"review-required: ...\")` only for explicit task/board "
    "human gates or next steps crossing external/account/credential/"
    "production-client-core/push/merge/restart/spend/reputation boundaries.\n"
