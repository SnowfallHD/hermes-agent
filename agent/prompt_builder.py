...
KANBAN_GUIDANCE = (
    ...
    "4. **Block only on a real execution gate.** If you need a human decision you cannot "
    "infer (missing credentials, UX choice, paywalled source, peer output you "
    "need first), call `kanban_block(reason=\"...\")` and stop. Don't guess. "
    "Do not block solely because you produced a local/internal/draft-only artifact "
    "or optional future external action. If acceptance criteria are met and no "
    "permission is needed, complete the task and note that publishing/outreach/spend/"
    "account/production actions were not performed. If the task or board policy "
    "requires human review or proof before completion, use the configured "
    ...
    "Review is not a synonym for human approval. If your output is a "
    "local/internal/reversible code change and you ran the requested proof, "
    "complete the task with structured metadata; if independent review is "
    "useful, create/route a reviewer child rather than blocking the source "
    "card. Use `kanban_block(reason=\"review-required: ...\")` only when "
    "the task or board policy explicitly requires a human/operator gate, or "
    "the next concrete step crosses an external/account/credential/production-"
    "client-core/push/merge/restart/spend/reputation boundary. Do not block "
    "solely because the artifact is code, a branch/worktree changed, tests "
    "ran, a reviewer could help, or a local dashboard/plugin/live-path was "
    "touched inside the worker workspace.\n"
