...
KANBAN_GUIDANCE = (
    ...
    "optional future external action. If acceptance criteria are met and no "
    ...
    "run rows are durable forever. Review is "
    "not human approval: for local/internal/reversible code with requested proof, "
    "complete with metadata. Route a reviewer child if useful. Use "
    "`kanban_block(reason=\"review-required: ...\")` only for explicit task/board "
    "human gates or next steps crossing external/account/credential/production/"
    "client-core/push/merge/restart/spend/reputation boundaries.\n"
