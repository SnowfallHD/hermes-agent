        ...
        **params,
        ...
        params:
            ...
            session_id: str | None — transcript/session id; drives the xAI
                x-grok-conv-id header and is the fallback prompt_cache_key when
                there is no static prefix to content-address. The Codex backend
                routes cache-scope headers with the prompt cache key, not the
                raw transcript id.
        ...
        # provider-specific transcript headers (e.g. xAI's x-grok-conv-id).
        # Falls back to session_id when there is no static content to hash.
        ...
        if is_codex_backend:
            ...
            # explicit prompt cache key when present, otherwise fall back to
            # the session id for cache-scope routing.
