...
CONFIGURABLE_TOOLSETS = [
    ...
    ("mind_events",     "🧠 Mind/Event Telemetry",      "record_mind_event thought-quality signals"),
...
_DEFAULT_OFF_TOOLSETS = {"moa", "homeassistant", "spotify", "discord", "discord_admin", "video", "video_gen", "x_search", "mind_events"}
...
def _static_toolset_tools(toolsets: Dict[str, dict], ts_key: str, visited: Optional[Set[str]] = None) -> Set[str]:
    """Return tools declared by ``toolsets.py`` without registry side effects.

    ``toolsets.get_toolset()`` intentionally merges in dynamically registered
    tools. That is correct for runtime schema loading, but it is too broad for
    reverse-mapping a platform composite back to user-toggleable toolset keys:
    late plugin discovery can attach GUI-only tools to a configurable group and
    make ``resolve_toolset('terminal')`` no longer be a subset of a static
    ``hermes-*`` composite. Use the static declaration for subset inference and
    fall back to the normal resolver only for unknown/dynamic names.
    """
    if visited is None:
        visited = set()
    if ts_key in visited:
        return set()
    visited.add(ts_key)
    ...
    for included in ts_def.get("includes", []):
        tools.update(_static_toolset_tools(toolsets, str(included), visited))
    return tools
...
def _get_platform_tools(
    ...
    *,
        ...
        if composite_tools:
            ...
            for ts_key, _, _ in CONFIGURABLE_TOOLSETS:
                if not _toolset_allowed_for_platform(ts_key, platform):
                    continue
                ts_tools = _static_toolset_tools(TOOLSETS, ts_key)
    ...
    platform_tool_universe = _static_toolset_tools(TOOLSETS, _default_ts) or set(resolve_toolset(_default_ts))
    configurable_tool_universe = set()
    for ck in configurable_keys:
        configurable_tool_universe.update(_static_toolset_tools(TOOLSETS, ck))
    claimed = set()
    for ts_key in enabled_toolsets:
        claimed.update(_static_toolset_tools(TOOLSETS, ts_key) or set(resolve_toolset(ts_key)))
