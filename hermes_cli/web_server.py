...
async def get_session_messages(session_id: str, profile: Optional[str] = None):
    ...
    try:
        ...
        if not sid:
            raise HTTPException(status_code=404, detail="Session not found")
        metadata = db.get_conversation_metadata(sid)
        messages = db.get_conversation_messages(sid)
        return {
            "session_id": metadata["latest_session_id"],
            "messages": messages,
            **metadata,
        }
    finally:
...
def _call_cron_for_profile(target_profile: Optional[str], func_name: str, *args, **kwargs):
    ...
    with _CRON_PROFILE_LOCK:
        ...
        old_get_hermes_home = getattr(cron_jobs, "get_hermes_home", None)
        token = set_hermes_home_override(str(home))
        cron_jobs.CRON_DIR = home / "cron"
        cron_jobs.JOBS_FILE = cron_jobs.CRON_DIR / "jobs.json"
        cron_jobs.OUTPUT_DIR = cron_jobs.CRON_DIR / "output"
        # cron.jobs imports get_hermes_home directly at module import time.
        # Some tests/tools intentionally reload hermes_constants after cron.jobs
        # is already imported, which leaves cron.jobs.get_hermes_home pointing at
        # a stale module-level override store. Patch the imported helper while
        # profile-routing so snapshots/config reads use the same home as storage.
        cron_jobs.get_hermes_home = lambda: home
        try:
            ...
            if old_get_hermes_home is not None:
                cron_jobs.get_hermes_home = old_get_hermes_home
