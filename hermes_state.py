...
class SessionDB:
    ...
    def get_compression_chain(self, session_id: str) -> List[str]:
        """Return the canonical compression chain containing ``session_id``.

        Callers may pass any raw segment id (root, middle continuation, or
        current tip) and receive the root-to-tip chain. Branch, delegate, and
        tool child sessions are deliberately excluded; only children of
        compression-ended parents are considered continuations. This mirrors
        :meth:`get_compression_tip`'s robust continuation heuristic rather than
        relying on brittle child/parent timestamp ordering.
        """
        if not session_id:
            return []
