...
class SlackAdapter(BasePlatformAdapter):
    ...
    async def send(
        ...
        try:
            ...
            auto_split = split_auto_parent_thread_content(
                content,
                thread_ts=thread_ts,
                enabled=auto_parent_thread_enabled(self.config.extra),
            )
            if auto_split:
    ...
    async def _observe_kryden_coordination_event(self, event: dict) -> None:
        ...
        Disabled by default. Coop's current cron/noise policy is one morning
        brief, one quiet 1 PM heartbeat, one evening brief, and bounty lane
        crons only. A live Slack/Viktor observer can create surprising replies
        outside that surface, so it must be explicitly re-enabled.
        """
        enabled = str(os.getenv("KRYDEN_SLACK_COORDINATION_OBSERVER_ENABLED", "false")).strip().lower()
    ...
    async def _handle_slack_message(self, event: dict) -> None:
        ...
        if event.get("bot_id") or event.get("subtype") == "bot_message":
            ...
            # Optional Kryden/Viktor exception. Disabled by default because Coop
            # wants noisy background/agent-to-agent reply surfaces off unless
            # explicitly re-enabled.
            raw_bot_text = event.get("text", "") or ""
            viktor_user_id = os.getenv("KRYDEN_VIKTOR_USER_ID", "U0B0YLS5MN3")
            viktor_direct_enabled = str(os.getenv("KRYDEN_VIKTOR_DIRECT_REPLY_ENABLED", "false")).strip().lower() in {"1", "true", "yes", "on"}
            viktor_directed_hermes = bool(
                viktor_direct_enabled
                and msg_user == viktor_user_id
        ...
        # Optional Viktor plain-text "@Hermes" / "Hermes" mention emulation.
        # Disabled by default under Coop's current noise policy; real Slack
        # mentions still work through the normal mention path.
        viktor_direct_enabled = str(os.getenv("KRYDEN_VIKTOR_DIRECT_REPLY_ENABLED", "false")).strip().lower() in {"1", "true", "yes", "on"}
        viktor_plain_hermes_ask = bool(
            viktor_direct_enabled
            and event.get("user") == os.getenv("KRYDEN_VIKTOR_USER_ID", "U0B0YLS5MN3")
            and re.search(r"(^|\s)@?Hermes\b", routing_text, re.I)
            and re.search(
                r"\?|\b(can you|could you|should we|need|thoughts|blocked|blocker|waiting on|approve|approval|have you spotted|want to make sure|if you spot|drop them|send them|summarize|summary|status)\b",
                routing_text,
                re.I,
            )
        )
