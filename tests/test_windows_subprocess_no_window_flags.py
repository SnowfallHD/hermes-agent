...
def test_tui_gateway_fuzzy_file_listing_hides_git_windows(monkeypatch):
    ...
    def fake_run(cmd, **kwargs):
        captured.append((cmd, kwargs))
        if cmd[:3] == ["git", "-C", "C:/repo"] and cmd[-1] == "--show-toplevel":
            return _Completed(stdout=b"C:/repo\n")
        if cmd[:3] == ["git", "-C", "C:/repo"]:
            return _Completed(stdout=b"src/main.py\0README.md\0")
        # Ignore unrelated background probes from tui_gateway daemon timers that
        # share the module-level subprocess object in this test process.
    ...
    git_captured = [
        (cmd, kwargs) for cmd, kwargs in captured if cmd[:2] == ["git", "-C"]
    ]
    assert [kwargs["creationflags"] for _, kwargs in git_captured] == [
        _CREATE_NO_WINDOW,
        _CREATE_NO_WINDOW,
    ]
