    ...
    def test_entire_tree_is_sigkilled_not_just_parent(self, monkeypatch):
        ...
        try:
            ...
            def _pid_dead(p: int) -> bool:
                ...
                # not reaped by its reparented parent yet). psutil.pid_exists()
                # uses os.kill(pid, 0) on POSIX; the test live-system guard
                # blocks os.kill for PIDs that have already died/reparented/reused
                # outside the pytest subtree, so inspect /proc directly on Linux.
                if sys.platform.startswith("linux"):
                    try:
                        with open(f"/proc/{p}/stat", encoding="utf-8") as fh:
