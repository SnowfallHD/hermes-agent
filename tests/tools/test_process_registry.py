    ...
    def test_entire_tree_is_sigkilled_not_just_parent(self, monkeypatch):
        ...
        try:
            ...
            def _pid_alive_no_signal(pid: int) -> bool:
                # psutil.pid_exists() uses os.kill(pid, 0) on POSIX. The test
                # live-system guard correctly blocks os.kill for PIDs that have
                # already died/reparented/reused outside the pytest subtree, so
                # inspect /proc directly on Linux to avoid turning a successful
                # kill into a guard failure.
            ...
            def _all_dead():
                return not any(_pid_alive_no_signal(p) for p in all_pids)
