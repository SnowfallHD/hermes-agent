...
def is_approved(session_key: str, pattern_key: str) -> bool:
    ...
    existing permanent command_allowlist entries continue to work after key
    migrations. Session approvals stay exact-key only: legacy regex-derived
    keys can collide across distinct patterns (for example, multiple ``find``
    guards), and approving one dangerous command for a live session must not
    silently approve a sibling pattern.
    """
