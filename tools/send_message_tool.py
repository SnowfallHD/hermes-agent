...
def _get_cron_auto_delivery_target():
    ...
    def _cron_delivery_value(name: str) -> str:
        # Cron delivery vars are task-local in the gateway, but cron CLI/test
        # paths still seed them through os.environ.  Unlike ordinary
        # HERMES_SESSION_* vars, an explicitly empty ContextVar here should not
        # suppress a same-process env fallback: it only means this context has no
        # cron delivery target yet.
        return (get_session_env(name, "") or os.getenv(name, "")).strip()

    platform = _cron_delivery_value("HERMES_CRON_AUTO_DELIVER_PLATFORM").lower()
    chat_id = _cron_delivery_value("HERMES_CRON_AUTO_DELIVER_CHAT_ID")
    if not platform or not chat_id:
        return None
    thread_id = _cron_delivery_value("HERMES_CRON_AUTO_DELIVER_THREAD_ID") or None
    return {
        "platform": platform,
        "chat_id": chat_id,
        "thread_id": thread_id,
