...
def _(rid, params: dict) -> dict:
    ...
    # Follow the compression/canonical-conversation chain to the live tip so a
    # resume on a rotated-out parent or stable root conversation id binds to the
    # descendant that actually holds post-compression turns. Auto-compression
    # ends the session and forks a continuation child; without this, resuming
    # the original id reloads the parent transcript and new turns can create a
    # visible fork. Skipped for lazy watch windows, which intentionally attach
    # to the exact child branch they were opened on.
    if found and not is_truthy_value(params.get("lazy", False)):
        try:
            metadata = db.get_conversation_metadata(target)
            tip = metadata.get("latest_session_id") or target
        except Exception:
            try:
